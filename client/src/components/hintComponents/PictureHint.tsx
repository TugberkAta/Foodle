  ...
  return (
    <div
      className={`md:w-96 w-10/12 mt-10 rounded-md border-2 border-gray-700 relative ${
        count < 4 ? "bg-gray-900" : "bg-none"
      }`}
    >
      <svg
        className={`absolute w-20 m-auto inset-0 fill-gray-700 ${
          count >= 4 ? "invisible" : "visible"
        }`}
      ...
      <img
        className={`w-96  ${count < 4 ? "invisible" : "visible"}`}
        src={foodData.foodImg}
        alt={foodData.imgAlt}
      />
    </div>
  );
